// frontend/src/components/NuevoEstadio.js
...
const NuevoEstadio = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await estadiosService.create(formData);
      await handleResponse(response);

      alert('¡Estadio creado exitosamente!');
      navigate('/dashboard');
      ...
      if (error.message.includes('ya existe')) {
        ...
        alert('Error al crear el estadio: ' + error.message);
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="form-container">
      <div className="form-card">
        <div className="form-header">
          <h2>Nuevo Estadio</h2>
          <p>Registrar un nuevo estadio en el sistema</p>
        </div>
